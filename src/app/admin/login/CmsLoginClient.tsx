...
import { User, Lock, Eye, EyeOff, ArrowRight, ShieldCheck, Database, FileText, BookOpen } from "lucide-react";
import { withBasePath } from "@/lib/basePath";
import { getCmsAuthed, setCmsAuthed, CMS_USERNAME, CMS_PASSWORD } from "@/lib/cmsAuth";
...
const features = [
  ...
  { Icon: Database, label: "Tersinkronisasi ke GitHub", sub: "Perubahan langsung ke repo" },
];
...
export function CmsLoginClient() {
  ...
  useEffect(() => {
    if (getCmsAuthed()) router.replace("/admin");
  }, [router]);

  function handleSubmit(e: FormEvent) {
    e.preventDefault();
    if (username.trim().toLowerCase() === CMS_USERNAME && password === CMS_PASSWORD) {
      setCmsAuthed();
      ...
      <div className="flex flex-1 flex-col items-center justify-center px-6 py-14 bg-[#111111]">
        ...
        <div className="w-full max-w-[340px]">
          <div className="mb-8">
            ...
            <p className="mt-1.5 text-sm text-white/40">Halaman khusus pengelola konten LMS CISDI.</p>
          </div>
